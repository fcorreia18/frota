...
<th {{ $attributes->merge(['class' => 'px-6 py-3 bg-cool-gray-50'])->only('class') }}>
    ...
    @else
        <button {{ $attributes->except('class') }}
            class=" flex items-center space-x-1 text-left text-xs leading-4 font-medium text-cool-gray-500 uppercase tracking-wider">

            <span style="font-size: 0.8rem">{{ $slot }}</span>

            <span>
                @if ($direction)
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
                        class="w-3 h-3 opacity-1">
